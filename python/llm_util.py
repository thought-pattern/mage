...
class SchemaGenerator(object):
    ...
    def get_schema(self) -> object:
        if self._type == OutputType.RAW.value:
            _return_value = self._get_raw_schema()
            return _return_value
        elif self._type == OutputType.PROMPT_READY.value:
            _return_value = self._get_prompt_ready_schema()
            return _return_value
        else:
            raise Exception(
    ...
    def _generate_schema(self, context: mgp_ProcCtx):
    ...
    def _get_raw_schema(self) -> mgp_Map:
        return {
    ...
    def _update_properties_dict(
        ...
        for property_name in graph_object.properties.keys():
                ...
                all_properties_dict[key] = [
                    {
                        ...
                        Parameter.TYPE.value: type(
                            graph_object.properties.get(property_name, False)
                        ).__name__,
                    }
                ]
                continue
            ...
            if property_name in [
                d.get(Parameter.PROPERTY.value, False)
                for d in all_properties_dict.get(key, False)
            ]:
                continue
...
def schema(
    ...
    _return_value = mgp_Record(schema=schema_generator.get_schema())
    return _return_value
