...
class CorrelationPopulation(Population):
    ...
    def _update_correlation(
        self, index: int, old_individual: Individual, nodes: List[int]
    ) -> int:
        ...
        for node in nodes:
            for neigh in self._graph[node]:
                if not processed[neigh]:
                    ...
                    S_prev = (
                        -1 if prev_individual[node] == prev_individual[neigh] else 1
                    )
                    S_next = (
                        -1 if next_individual[node] == next_individual[neigh] else 1
                    )
